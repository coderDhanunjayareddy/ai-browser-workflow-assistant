...
export async function executeAction(action) {
     ...
     */
    function waitForAny(selectors, timeoutMs = 10_000) {
        return new Promise((resolve) => {
            // Fast path.
            for (const sel of selectors) {
                const el = safeQuery(sel);
                if (el) {
                    resolve({ selector: sel, el });
                    return;
                }
            }
            const deadline = Date.now() + timeoutMs;
    ...
    try {
        switch (action_type) {
            ...
            case 'click': {
                ...
                function tryClick(candidate, resolvedSelector) {
                    ...
                    const topEl = (pointed instanceof HTMLElement ? pointed : null) ?? candidate;
                ...
                const attrValueMatch = target_selector.match(/\[(?:title|aria-label|placeholder|data-testid)=["']([^"']+)["']\]/);
                if (attrValueMatch) {
                ...
                const isTypeable = (e) => e instanceof HTMLInputElement ||
                    e instanceof HTMLTextAreaElement ||
                    (e instanceof HTMLElement && e.getAttribute('contenteditable') === 'true');
                const isSearchBox = (e) => {
                    if (!e)
                        return false;
                    if (e instanceof HTMLInputElement && e.type === 'search')
                        return true;
                    const role = e.getAttribute('role');
                    if (role === 'searchbox' || role === 'combobox')
                        return true;
            ...
            case 'fill': {
                ...
                function fillInput(inputEl, fillValue, resolvedSelector) {
                    inputEl.focus();
                ...
                function fillContentEditable(ceEl, text, resolvedSelector) {
                    ceEl.focus();
                    ceEl.textContent = '';
                ...
                if (!el) {
                    const attrSwap = target_selector
                        ...
                        .replace(/\[aria-label=(["'][^"']+["'])\]/, '[title=$1]');
                    if (attrSwap !== target_selector)
                        el = await waitForElement(attrSwap, 2_000);
                }
                ...
                const attrMatch = target_selector.match(/\[(?:title|aria-label|placeholder|data-testid)=["']([^"']+)["']\]/);
                if (attrMatch) {
            ...
            case 'wait': {
                ...
                if (target_selector && target_selector !== 'window') {
                    ...
                    let el = await waitForElement(target_selector, 1500);
                    if (el) {
