"""
V8.5 Governance Layer — GovernanceTimeline.

Chronological deque of contract lifecycle events.
Reuses the deque-based timeline pattern from V7.0/V7.5/V8.0.
"""
...
class GovernanceTimeline:
    ...
    def record(
        self,
        contract_id: str,
        event_type:  str,   # "created" | "revoked" | "expired" | "consumed"
        mission_id:  str = "",
        risk_level:  str = "LOW",
        source_type: str = "",
        approved:    bool = True,
    ...
    def missions_with_contracts(self) -> list[str]:
        with self._lock:
...
_timeline = GovernanceTimeline()


def record(contract_id: str, event_type: str, **kwargs) -> None:
    _timeline.record(contract_id, event_type, **kwargs)
...
def missions_with_contracts() -> list[str]:
    return _timeline.missions_with_contracts()
