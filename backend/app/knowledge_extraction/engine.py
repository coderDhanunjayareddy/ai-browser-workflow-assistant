...
class KnowledgeExtractionPipeline:
    ...
    def observe(
        ...
        *,
        ...
        records = extract_records(read, task, current_phase) if read and is_shadow_or_active("V50_EXTRACTION_ENGINE") else []
        ...
        knowledge = synthesize_knowledge(all_records, required, task) if is_shadow_or_active("V50_SYNTHESIS") else None
        synthesis_ms = int((time.perf_counter() - synthesis_started) * 1000)
        report_started = time.perf_counter()
        report = generate_report(knowledge) if is_shadow_or_active("V50_REPORT_ENGINE") else None
        ...
        return KnowledgePipelineSnapshot(
            ...
            completion_status={
                "read": bool(reads),
                "extract": bool(all_records) and validation["valid"],
