...
## Phase D — Adaptive Execution & Recovery Engine

**Date:** 2026-06-27
**Status:** Complete — improves real-browser execution reliability with ZERO new
architecture layers and ZERO breaking changes.

### Objective

Make the existing Playwright execution noticeably closer to production quality —
deterministic recovery, an adaptive locator chain, and first-class validation — WITHOUT
redesigning the gateway, planner, missions, authorization, governance, runtime, or
browser sync. Everything is ADDITIVE; the existing PlaywrightAdapter remains the single
execution adapter.

### Additivity & Backward-Compatibility Guarantee

Phase D is delivered as new sibling modules under `app/execution_gateway/browser/` plus
**opt-in flags** on the adapter (`adaptive`, `recovery`, `post_validation`). With the
flags OFF (the default), the adapter is BYTE-IDENTICAL to Phase C: `_execute` delegates
to the unchanged Phase C `_run`, and `_resolve`/`_strategy_for` delegate to the Phase C
resolver. Real execution turns Phase D ON via `run.execute_plan_with_browser` (which
still uses only the EXISTING gateway `start`/`resume` API — no gateway/runner/retry-engine
code changed). Result: **all 3,182 prior tests continue to pass.**

A 6-dimension multi-agent adversarial review (backward-compat, constraint-compliance,
failure/recovery correctness, resolver/validation correctness, observability, retry-policy)
returned **zero confirmed defects** after adversarial verification.

### 1. Execution Monitor (`monitor.py`)

Per-step `StepMonitorRecord`: start/finish time, elapsed, attempts/retries, validation
result, failure category, recovery used, locator strategy, screenshots. Thread-safe,
keyed by execution_id, queryable for the inspector/diagnostics APIs.

### 2. Failure Classification Engine (`failure_classes.py`)

18 deterministic `FailureCategory` values (ElementNotFound, ElementHidden,
DetachedElement, NavigationTimeout, PageCrash, DownloadTimeout, UploadFailure,
ValidationFailure, UnexpectedPopup, NetworkIdleTimeout, AuthenticationExpired, …). Each
has a `FailureProfile` with **severity** (TRANSIENT/RECOVERABLE/PERMANENT), **retryable**,
and **recommended_recovery** actions. `classify_failure` layers a keyword-first refinement
on top of the unchanged Phase C `errors.classify` — page-crash / popup / auth / hidden /
network-idle messages are detected from any base. NO AI.

### 3. Recovery Engine (`recovery.py`)

Deterministic recovery executed BEFORE a retry, driven by the failure profile:
ElementNotFound → wait + refresh locator; ElementHidden → scroll into view; Detached →
re-query; NavigationTimeout → wait for network idle; ValidationFailure → re-read page;
PageCrash → reload; UnexpectedPopup → dismiss popup. Best-effort and fully guarded — a
recovery never raises into the runner, and returns `recovered=False` on a bad/None session.
Permanent failures get NO recovery (fail immediately).

### 4. Adaptive Locator Strategy (`adaptive_resolver.py`)

`AdaptiveResolver` extends the Phase C `ElementResolver` with the chain
`selector → testid → aria_label → aria → role → label → placeholder → text → id → name
→ css → xpath` plus optional strict-uniqueness validation (`resolve_strict` raises on 0 or
>1 matches when `strict` is set). The relative order of every Phase C strategy is
preserved, so existing param sets resolve identically. The winning strategy is recorded;
metrics expose per-strategy usage rates.

### 5. Execution Validation (`execution_validation.py`)

First-class post-action validation declared per step under `parameters["validate_after"]`:
`url_contains`, `url_changed`, `text_contains`, `text_absent`, `exists`, `gone`,
`value_equals` (TYPE), `filename_visible` (UPLOAD), `file_exists` (DOWNLOAD). Absent →
no-op (performed=False, passed=True), so existing plans are unaffected. A failed
post-validation makes the step's `validation_passed=False` (the gateway then fails the
step) after a bounded re-read+revalidate recovery.

### 6. Retry Policy V2 (in `playwright_adapter._run_adaptive`)

Retries happen ONLY when the failure class is retryable AND the retry budget remains —
**permanent failures fail immediately at attempt 1**. Deterministic recovery runs before
each retry; retries are strictly bounded by `RetryConfig` (never infinite). Reuses the
EXISTING Retry Engine; no retry-engine code changed.

### 7. Execution Timeline (`exec_timeline.py`)

Per-execution, per-step queryable timeline recording all 8 lifecycle events: planned,
started, retried, recovered, validated, completed, failed, rollback. Additive — does not
touch the Phase B gateway timeline.

### 8. Execution Metrics (`metrics.py`)

Aggregate analytics: average retries, recovery success %, validation success %,
locator-strategy distribution, average execution time, step success rate, failure
distribution. Thread-safe, safe division-by-zero.

### 9. Browser Diagnostics (`diagnostics.py`)

Read-only surface: page url / title / active frame / active tab / locator strategy used /
recovery history / validation history / retry history / last screenshot METADATA (path +
filename only — never image bytes).

### 10. Real Browser Certification

Deterministic local-HTML E2E (no internet): forms, tables, uploads, downloads, iframes,
multiple tabs, dialogs, navigation, validation, retries, and recoveries — including a
recovery-driven SUCCESS (a click on a late-appearing element fails, recovers, retries,
and succeeds).
...
| GET | /gateway/browser/monitor/{execution_id} | per-step monitor summary |
| GET | /gateway/browser/timeline/{execution_id} | per-step execution timeline |

All 8 prior gateway endpoints and the 2 Phase C browser endpoints are unchanged.

### Files Created

| File | Purpose |
|------|---------|
| browser/failure_classes.py | 18-category failure classification + profiles |
| browser/recovery.py | deterministic RecoveryEngine |
| browser/adaptive_resolver.py | extended locator chain + strict uniqueness |
| browser/execution_validation.py | first-class post-action validation |
| browser/monitor.py | ExecutionMonitor (per-step records) |
| browser/metrics.py | ExecutionMetrics (aggregate analytics) |
| browser/exec_timeline.py | per-step ExecutionTimeline (8 events) |
| browser/diagnostics.py | read-only diagnostics (metadata only) |
| tests/unit/test_phased_*.py | 8 unit-test files (150 tests) |
| tests/integration/test_phased_gateway_integration.py | gateway + Phase D adapter (fake browser) |
| tests/integration/test_phased_real_browser.py | real-browser certification (26 incl. integration) |
| validate_phased.py | 1663-check validation suite (incl. real browser) |
| benchmark_phased.py | benchmark suite |

### Files Modified (additive only)

| File | Change |
|------|--------|
| browser/playwright_adapter.py | opt-in Phase D flags + `_run_adaptive` (flags off = Phase C identical) |
| browser/run.py | enable Phase D for real execution; emit planned/rollback timeline events |
| browser/capabilities.py | add EXTENDED_RESOLUTION_PRIORITY (superset of the unchanged Phase C priority) |
| browser/failure_classes.py | keyword-first `_refine` (hidden/crash/popup/auth detected from any base) |
| api/routes/gateway.py | add 4 read-only diagnostics/metrics endpoints |

The Phase C resolver.py and its RESOLUTION_PRIORITY are untouched; the gateway, dispatcher,
runner, retry engine, planner, missions, authorization, governance, runtime, and browser
sync are untouched.
...
| Suite | Tests | Result |
|-------|-------|--------|
| Unit: failure_classes | 22 | PASS |
| Unit: recovery | 14 | PASS |
| Unit: adaptive_resolver | 23 | PASS |
| Unit: execution_validation | 27 | PASS |
| Unit: monitor | 12 | PASS |
| Unit: metrics | 14 | PASS |
| Unit: timeline + diagnostics | 21 | PASS |
| Unit: adapter (Phase D) | 17 | PASS |
| Integration: gateway + Phase D (fake) | 18 | PASS |
| Real-browser certification (chromium) | 8 | PASS |
| **Phase D total** | **176** | **176/176 PASS** |
| Cross-milestone (all V6.5-Phase D) | 3358 | 3358/3358 PASS |

### Validation Results

**1663/1663 checks pass** across 22 sections: package structure, all enums, the full
profile table, a 35+20-case classification matrix, recovery per category (with guarded
no-crash), the complete adaptive builder matrix (12 strategies × 10 values) + Phase C
compatibility + strict uniqueness, all post-action validation checks, monitor, metrics
(with divide-by-zero safety), timeline (8 events), diagnostics (metadata-only), adapter
Phase D engagement + Retry Policy V2 (transient retried & bounded, permanent immediate) +
first-class validation, backward-compatibility (flags off = Phase C; Phase C resolver
untouched), the additive REST endpoints, a static safety scan (no LLM/OCR/Vision/
self-healing/Selenium/Puppeteer; no randomness in recovery/resolution/classification),
and a real-chromium certification (recovery-driven success).

### Benchmarks

| Benchmark | p95 | Target |
|-----------|-----|--------|
| B1: classify_failure() | 0.008ms | 0.5ms |
| B2: recovery.recover() | 0.006ms | 1ms |
| B3: adaptive resolve | 0.002ms | 0.5ms |
| B4: execution_validation | 0.003ms | 1ms |
| B5: monitor start+finish | 0.003ms | 0.5ms |
| B6: metrics record+get | 0.003ms | 1ms |
| B7: adapter Phase D click (fake) | 0.059ms | 2ms |
| adapter Phase C click (comparison) | 0.006ms (INFO) | — |
| navigate (real, Phase D) | 18.3ms (INFO) | — |

Phase D adds ~0.05ms of in-process overhead per action (monitor/metrics/timeline/
validation bookkeeping) — negligible. Real-browser latency is dominated by Playwright
actionability, unchanged from Phase C; gateway overhead is unchanged.

### Reliability Improvement (the point of this phase)

Phase D demonstrably improves success on real pages: a click on a late-appearing or
initially-hidden element — which would FAIL under Phase C — now classifies the failure,
runs a deterministic recovery (wait / scroll / re-query / network-idle / re-read), retries
within a strict bound, and SUCCEEDS. Recovery success rate, validation success rate, and
per-locator-strategy success are exposed via metrics; per-step recovery/validation/retry
history is exposed via diagnostics.

### Known Limitations

1. Recovery and resolution are intentionally deterministic — no AI selector self-healing
   (a hard constraint). A truly-missing element still fails after bounded recovery.
2. Iframe support is basic (top-level content); deep frame-switching is future work.
3. Multi-tab/popup orchestration is basic — new windows from `target=_blank` are not
   auto-registered into the session (the session tab-registry API supports manual
   multi-tab; certified at the session level).
4. UPLOAD/DOWNLOAD are adapter capabilities not yet emitted by the V9.0 planner's
   ActionType; they are certified by driving the adapter methods directly.
5. State is in-memory (monitor/metrics/timeline) — no durable persistence yet.

### Production Readiness

This phase moves real Playwright execution materially closer to production quality:
deterministic recovery, an adaptive locator chain with uniqueness validation, first-class
post-action validation, full per-step observability (monitor/metrics/timeline/
diagnostics), and a strict Retry Policy V2 — all behind the unchanged Approval →
Governance → Authorization → Gateway safety chain, with no new architecture layer and no
breaking API changes. Recommended next hardening: durable persistence for metrics/monitor,
deeper iframe/multi-window orchestration, and a planner extension to emit
upload/download/scroll actions natively.

## Phase E — Website Intelligence & Semantic DOM Understanding

### Goal

Give the assistant a deterministic, read-only understanding of *what a page is made of*:
convert a raw DOM tree into a semantic UI model (forms, tables, navigation, dialogs,
cards, dashboards, ...) plus an interactive-element registry with reusable locators and
advisory execution hints. This is the "perception" layer the planner/gateway can consult;
it never acts on the page.

### Hard Constraints (all honored)

- **Deterministic, DOM-driven** — keyword/tag/role/class rules only. Same input -> same
  output, byte-for-byte (verified: 64 determinism checks across 8 fixtures).
- **No AI / LLM / OCR / Vision / ML / embeddings / no self-healing** — enforced by a static
  safety scan over every module (forbidden-token sweep, part of the validation suite).
- **Read-only** — Website Intelligence performs NO browser actions. The only browser touch
  is a single read-only `page.evaluate(CAPTURE_JS)` DOM snapshot; the safety scan forbids
  `.click(/.type(/.fill(/.goto(/.set_input_files(/.press(/.hover(` etc. in all WI modules.
- **Purely additive** — no existing subsystem was modified in behavior. Gateway, Planner,
  Playwright Adapter, Adaptive Resolver, Recovery Engine, Runtime, Mission, Browser Sync,
  Authorization, Approval, Governance, Decision Center, Trust Engine are all unchanged.
  Integration points are new optional keys / new endpoints only.

### Architecture

New self-contained package `app/website_intelligence/` (12 modules) + one REST router.
Everything operates on a serializable `DomNode` snapshot, so every analyzer is
browser-free and unit-testable; `from_html()` (stdlib `html.parser`) builds snapshots for
fixtures, `capture(page)` builds them from a live Playwright page via one read-only
`evaluate`.

| Module | Responsibility |
|---|---|
| `models.py` | `DomNode` + domain models; `SemanticType` (29), `ElementCategory`, `Priority` |
| `dom_snapshot.py` | `CAPTURE_JS` (read-only), `capture/from_dict/from_html` |
| `semantic_analyzer.py` | `classify()` (~27 structures) + `analyze_page()` -> `PageModel` tree + type census |
| `locator_builder.py` | `build_locator()` -> reuses `EXTENDED_RESOLUTION_PRIORITY` strategy order |
| `form_intelligence.py` | fields, labels, required, password/file/date, checkbox/radio groups, submit/reset, validation hints |
| `table_intelligence.py` | headers, sortable columns, row count, pagination/search/filters/selection, action/export buttons |
| `navigation_intelligence.py` | primary/secondary nav, breadcrumbs, tabs, menus, sidebars, active page, nested hierarchy |
| `dialog_intelligence.py` | modal/confirmation/alert/toast/drawer/popup/overlay; blocking, dismissible, buttons |
| `interactive_registry.py` | `InteractiveElement` registry: semantic_id/role/category/priority/visibility/enabled/locator/validation_strategy |
| `execution_hints.py` | advisory-only hints (loading indicator, preferred validation, expected nav/upload/download/dialog) |
| `analyzer.py` | facade: `analyze` / `analyze_html` / `analyze_live` -> `WebsiteIntelligenceResult` + stats |
| `inspector.py` | REST-queryable slices (semantic_tree/forms/tables/dialogs/navigation/registry/hints/locators/summary) |

**Locator reuse (no duplication):** `locator_builder` imports
`EXTENDED_RESOLUTION_PRIORITY` from the Phase D adapter capabilities and emits candidate
strategies in that exact priority order, so a WI locator is directly consumable by the
existing Adaptive Resolver. The validation suite asserts every emitted locator's params
are a subset of the resolver's known strategies (+`role_name`).

### REST surface (additive)

- `POST /website-intelligence/analyze` — body `{html|snapshot, url?, title?}` -> full result
  (thread-safe, browser-free; the primary path).
- `GET /website-intelligence/live/{execution_id}` and `/{section}` — analyze the live
  session page. Because Playwright's sync API is greenlet/thread-affine, when a live read is
  attempted from a worker thread the endpoint returns **409 with guidance** to capture a
  snapshot and POST it to `/analyze` (documented limitation, not a failure).
- Additive pointers: `GET /mission/{id}/inspect` gains an optional `website_intelligence`
  pointer; gateway browser diagnostics gains a `website_intelligence` availability block.

### Integration (additive only)

`main.py` includes the new router; `diagnostics.py`, `schemas/mission.py`, and
`api/routes/mission.py` gained optional guarded keys. 77 pre-existing
gateway/mission/diagnostics tests still pass unchanged.

### Verification

| Suite | Result |
|---|---|
| Unit tests (`test_phasee_*` incl. edge-cases) | 166 passed |
| Integration + real-browser (`test_phasee_*`) | 22 passed |
| Validation suite (`validate_phasee.py`) | **1832 / 1832 checks passed** (target >= 1800) |
| Benchmarks (`benchmark_phasee.py`) | 14 / 14 passed |
| Full backend regression | 3545 passed (only the pre-existing same-microsecond `test_v46_snapshot` flake, which passes in isolation) |

The validation suite covers: package structure, DOM snapshot, classification matrix
(tags/roles/classes), page model, locator matrix, form/table/navigation/dialog
intelligence, registry, hints, analyzer/inspector, determinism, REST endpoints,
additivity, a static safety scan (forbidden AI/ML/Vision/action/nondeterminism tokens
across all 12 modules), and a stress/invariants sweep (synthetic pages 1..60).

### Performance

Target: semantic analysis under 10ms on a medium page. On a representative ~195-node
medium page (3 forms, 2 tables, 6 cards, nav + dialog):

| Stage | median | p95 | budget |
|---|---|---|---|
| Full `analyze()` | 5.8ms | 7.9ms | < 10ms |
| `analyze_page()` (semantic analyzer) | 1.1ms | 1.8ms | < 8ms |
| forms / tables / nav / dialogs / registry | <= 1.0ms each | <= 1.7ms | — |
| inspector slices / `to_dict()` | < 0.25ms | — | — |

Memory ~115 KB / 195 nodes. Scaling is near-linear (195 -> 6.5ms, 315 -> 10.0ms,
629 -> 20.7ms, 1257 -> 42.2ms — no O(n^2)). Hot-path optimizations: lowercased-class /
class-list caching on `DomNode`, short-circuiting `text_content()` at its char limit,
folding the type census into the single tree-build pass, gating class-substring branches
in `classify()` behind a `has_cls` check, and collapsing the table/nav analyzers' multiple
full-tree scans into single passes.

### Adversarial review

A 17-agent adversarial review (5 review dimensions x parallel verification) confirmed
**11 correctness edge-cases** (constraints + additivity reviewers found zero issues). All
11 were fixed and locked behind regression tests (`test_phasee_edge_cases.py`):

1. CSS locator skips Tailwind-style class tokens (`w-1/2`) and digit-leading ids -> `[id=]`.
2. XPath values are quoted via a safe literal helper (handles embedded `"`).
3. Pagination no longer over-matches the `"pag"` substring (precise `pagination`/`paging`/
   `page navigation` tokens; "In-page navigation" stays NAVIGATION).
4. `_is_reset` no longer steals submit buttons whose label contains "clear"/"reset"
   (a typeless `<button>` is a submit; only explicit `reset`/`type=button` clear controls).
5. Table widget container no longer balloons to `<main>`/`<section>` (bounded climb to a
   grid-container *class*), so sibling tables don't share pagination/search/export.
6. Grid-container class match is token-aware ("playlist" no longer matches "list").
7. Sibling-table pager attribution removed (no whole-subtree `find_first` during scoping).
8. `_is_sortable` requires a sort signal (a plain header button is not "sortable").
9. `_row_count` sums all `<tbody>` groups and only subtracts true header rows.
10. Dialog close detection treats bare "x" as exact-match (not substring -> "Next"/"Exit").
11. Dialog label never uses an `aria-labelledby` ID reference as human-readable text.

### Known Limitations

1. Live `/live` reads require the Playwright page to be reachable from the calling thread;
   the thread-safe path is POST `/analyze` with a captured snapshot (documented; 409 guides).
2. Classification is heuristic by design (no ML) — exotic/ARIA-poor markup may be
   under-classified; confidence scores expose this.
3. Hints are strictly advisory — the planner/gateway may ignore them; WI never acts.

### Production Readiness

Phase E adds a deterministic perception layer behind a clean read-only boundary: no AI, no
browser actions, no architecture change, no breaking APIs. It is consumable today via REST
and via the additive mission/diagnostics pointers, and is fast enough (sub-10ms on medium
pages) to run inline before planning. Recommended next steps: a planner adapter that
consumes WI locators/hints directly, and deeper shadow-DOM / iframe snapshot capture.

## Phase F — Real Website Certification & Reliability Program

### Goal

Transition the (feature-complete) platform from "feature complete" to "production-quality
browser automation" by **measuring, improving, and documenting execution reliability** on
real websites. No new architecture: only certification infrastructure, reliability metrics,
diagnostics, and targeted fixes discovered through testing. Everything additive; the
Execution Gateway / Planner / Playwright Adapter / Adaptive Recovery / Runtime / Authorization
are unchanged.

### Architecture (what was added — `app/certification/`)

The certification runner is **not** a new orchestration layer: it builds READY
`ExecutionPlan`s (with authorization + mission) and delegates to the UNCHANGED gateway
(`execute_plan_with_browser` for real chromium, `gateway.start` with the deterministic mock
adapter otherwise), then evaluates outcomes.

| Module | Responsibility |
|---|---|
| `models.py` | `CertificationScenario` / `SuccessCriterion` / `CertificationResult` / `WorkflowOutcome` |
| `fixtures.py` | 18 local HTML apps + a loopback `FixtureServer` (offline regression baseline) |
| `scenarios.py` | 24 declarative workflow certifications (planner ActionType steps + criteria) |
| `reliability.py` | workflow success rate + duration/semantic **percentiles (p50/p95/p99)**, merging Phase D step metrics |
| `failure_catalog.py` | structured, deduplicated failure knowledge base (category/site/workflow/repro/recovery/resolution/first+last seen) |
| `trace.py` | consolidated **workflow trace** (lifecycle + recovery + validation + retry + locator + semantic-snapshot pointer) |
| `runner.py` | thin harness: build plan → delegate to gateway → evaluate criteria → record reliability + catalog |
| `report.py` | certification report (supported/unsupported/limitations/failure-patterns/recommendations) |
| REST `api/routes/certification.py` | `/certification/{scenarios,reliability,failures,run,report,workflow-trace}` |

### Local Certification Fixtures (the regression baseline)

18 deterministic, offline HTML applications with stable ids/testids/labels and deterministic
post-action state: login, registration, dashboard, CRUD table, search & filters, file upload,
file download, pagination, modal dialog, multi-step form, infinite scroll, nested navigation,
tabs, accordion, dynamic loading, toast, confirmation dialog, drag-and-drop (analysis-only).
Each is also a valid Website Intelligence input.

### Workflow Certifications

24 end-to-end scenarios cover every required workflow: fill & submit a form (login +
registration), search, filter, edit a table row, upload, download, navigate multiple pages,
dashboard refresh, nested navigation, confirmation dialog, modal edit, recover from a delayed
element, resume after a transient failure, dynamic loading with explicit wait, pagination,
multi-step form, tabs, accordion, toast, infinite scroll, a **bounded-failure** negative path,
an **ambiguous-locator guard** negative path, and drag-and-drop analysis. Each verifies
expected state after completion (state, post-validation, content, recovery-used, semantic
structure, or bounded/typed failure).

### Targeted reliability fix discovered through testing

Certification of the resolver boundary revealed a real defect: an **ambiguous locator** (a
strict locator matching >1 element — e.g. text `"Edit"` on a two-row CRUD table, or a
Playwright strict-mode violation) was misclassified as `ElementNotFound` and sent through a
`WAIT → REFRESH_LOCATOR` recovery loop **three times**. Waiting/refreshing can never make two
elements become one, so this wasted ~2.2 s and produced a misleading diagnostic.

Fix (additive, in `failure_classes.py`): a new **permanent** `AmbiguousLocator` category with
`recovery = (NONE,)`, plus a deterministic refine rule that recognises strict-uniqueness /
strict-mode-violation messages with a match count ≥ 2 (a "matched 0 elements" strict failure
remains the recoverable `ElementNotFound`). Result: **attempts 3 → 1, ~2.2 s → ~1.0 s**, an
accurate category, and zero wasted recovery — verified end-to-end and certified by the
`cert-ambiguous-guard` scenario (fails fast in ~0.84 s vs the missing-element path's ~2.7 s).

### Diagnostics improvement — workflow trace

`certification/trace.py` adds `workflow_trace(execution_id)` (REST: `/certification/workflow-
trace/{id}`) consolidating the existing Phase D observability into one view: per-step lifecycle,
recovery timeline, validation history, retry history, locator strategy used, execution summary,
and a semantic-snapshot pointer. Pure aggregation of data the platform already records;
read-only; honours the Phase E thread-affinity constraint.

### Verification

| Suite | Result |
|---|---|
| Unit (framework + mock runner) | 35 passed |
| Real-browser certification (`test_phasef_certification_real_browser.py`) | **30 passed** (24 scenarios + 6 dedicated) — **100% certification** |
| Validation (`validate_phasef.py`) | **1567 / 1567 checks passed** |
| Benchmarks (`benchmark_phasef.py`) | 6 / 6 passed |
| Full backend regression | 3584 passed (+30 cert real-browser) — only the pre-existing same-microsecond `test_v46_snapshot` flake (passes in isolation) |

### Reliability metrics (real chromium, local baseline)

- **Workflow success rate: 100%** (24/24); **zero flakiness** across 30 repeated runs of the
  timing-sensitive scenarios.
- Step success 100%, **recovery success 100%** (recovery engaged on delayed/hidden elements),
  **post-action validation 100%**.
- Locator strategies exercised: testid / text / css.
- Real workflow duration percentiles (per-scenario, includes browser launch): p50 ≈ 1.26 s,
  p95 ≈ 1.45 s. Mock-pipeline workflow p95 ≈ 2 ms. Semantic analysis p95 ≈ 2.3 ms.

### Performance (percentiles)

`benchmark_phasef.py` reports p50/p95/p99 for semantic analysis (p95 ≈ 2.3 ms), planning
(p95 ≈ 0.1 ms), mock workflow execution (p95 ≈ 2 ms), reliability rollup (p95 ≈ 0.2 ms),
report generation (p95 ≈ 0.7 ms), and failure classification (p95 ≈ 0.013 ms), plus full-suite
and real-browser workflow-duration percentiles as INFO.

### Known limitations

1. Upload/download file mechanics have no planner `ActionType` (documented Phase D gap); those
   scenarios certify page-load + semantic structure through the plan path, and the actual file
   transfer is certified at the adapter level (`PlaywrightAdapter.upload/download`) in the
   real-browser suite.
2. Drag-and-drop is analysis-only — no synthetic drag gesture is performed (no browser autonomy).
3. Certification fixtures are local/offline by design (deterministic, reproducible); live public
   sites are out of scope for the regression baseline.

### Production Readiness

Reliability is now **measurable, reproducible, and regression-protected**: a declarative
certification framework with a 18-pattern fixture baseline, workflow-level metrics with
percentiles, a structured failure catalog, a consolidated workflow trace, and a real-browser
certification suite that gates regressions at 100%. The one defect surfaced by certification
(ambiguous-locator misclassification) was fixed additively and is itself certified. The
platform transitions from "feature complete" to production-quality browser automation with no
architectural change and no breaking APIs. Recommended next steps: extend the planner with
native upload/download/scroll actions, add a curated set of resilient public-site smoke
certifications, and persist reliability metrics/catalog durably.

### Live certification dry run (spectropy.com) + popup hardening

A production dry run was executed against a real authenticated site (spectropy.com → the
"Spectropy OS" LMS) to discover pipeline weaknesses before the next phase. Steps 1–3 passed
(home + login + course search/selection); Steps 4–5 were intentionally **not** executed
because they would mutate a live production course (≈10 enrolled schools) and upload a
personal file — outside the scope of a diagnostic dry run. End-to-end status: **FAIL**
(workflow does not complete as specified), with four catalogued blockers:

1. **Pop-up-launched service (root-caused & fixed).** The "Spectropy OS" launcher calls
   `window.open(...)`; when not in a trusted gesture the browser blocks it, orphaning the
   launch. The browser **session** now (a) registers real popups it observes
   (`context 'page'` event + `page.opener()` discriminator, so explicit `new_page()` is
   never mistaken for a popup) exposing `popup_count()` / `latest_popup()` /
   `follow_latest_popup()`, and (b) records intended `window.open` targets via a behaviour-
   preserving init-script shim, exposed as `intended_popup_urls()`, so a **blocked** popup's
   URL is recoverable and the pipeline can navigate directly. Verified end-to-end against the
   live launcher (blocked popup → captured URL → direct nav → LMS login reached) and by
   `tests/integration/test_popup_handling_real_browser.py` (4 tests). Purely additive;
   `to_dict()` gains a `popups` count; the manual `register_tab`/`switch_tab` API is unchanged
   (`register_tab` is now dedup-aware). **Regression risk: Low** — full suite 3618 passed.
2. **Ambiguous "Content" button.** The FF_G6_BIO search returns three cards with identical
   "Content" buttons; an unscoped locator hits a strict-mode violation — the `AmbiguousLocator`
   class added earlier in Phase F. Mitigation: scope to the card whose metadata contains
   "Platform". No new code.
3. **Workflow/UI vocabulary mismatch.** Step 4's "Serial No 2" has no anchor in the live
   content tree (chapter names + topic buttons, no "Serial No" labels); it is only
   *interpretable* as the 2nd topic node. Resolution is a workflow-spec correction (address
   nodes by stable name/testid), not a code change.
4. **Production-mutation safety boundary.** Creating content and uploading a personal PDF to a
   live course is a hard-to-reverse, outward-facing action; certification of mutating flows
   must target a dedicated test course + fixture file, never production + personal files.

New session APIs (all additive, `session.py`): `popup_count()`, `latest_popup()`,
`follow_latest_popup()`, `intended_popup_urls()`, and a `popups` key in session info.
