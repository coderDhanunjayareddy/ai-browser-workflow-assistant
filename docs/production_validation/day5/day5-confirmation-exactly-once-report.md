...
17. The next run passed destination verification and local file binding but timed out waiting for a chooser after clicking `[data-testid="list-item-24"]`. Direct live-DOM inspection proved that selector was a chat/feed row whose descendant promotional message contained `documents`; it was not the attachment menu item. The real control is uniquely exposed as `role="menuitem"`, accessible name `Document`. The generic correction rejects long descendant-derived accessibility names, requires a concise exact insertion identity, boosts composer/menu insertion controls so bounded extraction cannot evict them, and preserves the selected control's exact name and role in the canonical contract. A row containing `attached document`, `download file`, and the requested filename now loses to the real `Attach` button. Backend focused tests pass 65/65, extension tests pass 209/209, and TypeScript validation passes. No chooser or file selection occurred in the failed run. The backend-only contract correction is active under the matching build `stabilization-20260822T072910Z` on PID `22208`, so no further extension reload is required.
...
- Latest exact insertion-identity runtime: `v0.4.0`, workspace `c2bb7f4-dirty`, build `stabilization-20260822T072910Z`, PID `22208`.
