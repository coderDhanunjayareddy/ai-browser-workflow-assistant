...
export function exactOpenOnlyCompletion(
  ...
  const inferredKind = semanticTargetKind(String(action.grounding?.semantic_kind || ''))
  ...
  const observedName = String(trace.exact_observed_name || signals.exact_observed_name || '').trim()
...
function semanticTargetKind(value: string): string {
